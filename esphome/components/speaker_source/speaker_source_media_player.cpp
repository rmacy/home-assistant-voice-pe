...
void SpeakerSourceMediaPlayer::handle_speaker_playback_callback_(uint32_t frames, int64_t timestamp, uint8_t pipeline) {
  ...
  // The decoder can go idle and move active_source to last_source while the
  // speaker still has queued frames. Keep accounting those physical playback
  // callbacks against the source until the final frame drains.
  media_source::MediaSource *active_source = ps.active_source.load(std::memory_order_relaxed);
  media_source::MediaSource *playback_source =
      active_source != nullptr ? active_source : ps.last_source;
  if (playback_source == nullptr) {
    return;
  }
  ...
  if (source_frames > 0) {
    ...
    playback_source->notify_audio_played(source_frames, timestamp);
  }
}
...
void SpeakerSourceMediaPlayer::loop() {
  ...
  } else if ((announcement_playlist_active ||
              ann_ps.pending_frames.load(std::memory_order_relaxed) > 0) &&
             old_state == media_player::MEDIA_PLAYER_STATE_ANNOUNCING) {
    // The decoder is done, but physical audio callbacks have not consumed all
    // frames handed to the speaker yet. Remain ANNOUNCING so voice-assistant
    // follow-up listening cannot start over its own queued response.
    this->state = media_player::MEDIA_PLAYER_STATE_ANNOUNCING;
  } else {
